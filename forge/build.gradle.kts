...
loom {
    forge {
        ...
        mixinConfig(
            "hextrace-common.mixins.json",
            "hextrace-forge.mixins.json",
        )
    }
    ...
    runs {
        for ((platform, outputProject) in arrayOf(
            ...
            register("${platform}Datagen") {
                ...
                property("hextrace.apply-datagen-mixin", "true")
                property("hextrace.$platform-datagen", "true")
            }
        }
    }
}

hextraceModDependencies {
...
dependencies {
    ...
    modImplementation(libs.inline.forge) { isTransitive = false }
