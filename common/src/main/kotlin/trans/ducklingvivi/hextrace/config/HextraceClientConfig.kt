package trans.ducklingvivi.hextrace.config
...
object HextraceClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HextraceServerConfig.holder.config = HextraceServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HextraceServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = Hextrace.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use HextraceServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HextraceServerConfig.ServerConfig()
    }
